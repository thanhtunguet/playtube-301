...
if (empty($error)) {
    ...
    $video_title = addToHashTags(PT_Secure($_POST['title'],1));
    $video_description = addToHashTags(PT_Secure($_POST['description'],1));
    $data_insert      = array(
        'video_id'    => $video_id,
        'user_id'     => $user->id,
        'title'       => $video_title,
    ...
    if ($video_ok == true) {
	    ...
	    if ($insert) {
            ...
            if ($_POST['video-type'] == 'short' && !empty($_POST['video_url']) && preg_match("/(youtu.*be.*)\/(shorts)\/(.*?((?=[&#?])|$))/m", $_POST['video_url'], $match) && !empty($match[3]) && $pt->config->youtube_short == 'on') {
                ...
                $response = http_request_call(0, 'https://api.vevioz.com/file/mp4/' . $id, 0, 0, 0);
                $re = '#<a\s+(?:[^>]*?\s+)?href=(["\'])(.*?)\1#';
                preg_match_all($re, $response, $matches, PREG_SET_ORDER, 0);
                if (!empty($matches)) {
                    $update = array();
                    foreach ($matches as $key => $value) {
                        if (!empty($value[2]) && strpos($value[2], $id . '/mp4')) {
                            $video_url = 'upload/videos/' . date('Y') . '/' . date('m').'/' .PT_GenerateKey() . ".mp4";
                            file_put_contents($video_url, file_get_contents($value[2]));
