        ...
        foreach ($submit_data as $key => $value) {
            ...
            if ($key == 'ftp_upload') {
                if ($value == "on") {
                    ...
                    if ($pt->config->spaces == "on") {
                        $update = $db->where('name', 'spaces')->update(T_CONFIG, array('value' => "off"));
                    }
                    if ($pt->config->wasabi_storage == "on") {
                        $update = $db->where('name', 'wasabi_storage')->update(T_CONFIG, array('value' => "off"));
                    }
                    if ($pt->config->backblaze_storage == "on") {
                        $update = $db->where('name', 'backblaze_storage')->update(T_CONFIG, array('value' => "off"));
                    }
                }
            }
            if ($key == 's3_upload') {
...
if ($first == 'testS3') {
    ...
    try {
        ...
        if (!empty($buckets) && in_array($pt->config->s3_bucket_name, $buckets)) {
            $data['status'] = 200;
            $array          = array(
                'upload/photos/d-cover.jpg',
                'upload/photos/d-avatar.jpg',
...
if ($first == 'test_wasabi') {
    ...
    try {
        ...
        $s3::$isWasabi = true;
        $buckets = $s3->listBuckets();
        if (!empty($buckets) && in_array($pt->config->wasabi_bucket_name, $buckets)) {
            $data['status'] = 200;
            $array          = array(
                'upload/photos/d-cover.jpg',
                'upload/photos/d-avatar.jpg',
...
if ($first == 'test_spaces') {
    ...
    include_once("assets/libs/spaces/vendor/autoload.php");
    try {
...
if ($first == 'monetization' && !empty($_POST['id']) && !empty($_POST['a'])) {
    ...
    if ($request === true) {
        ...
        if ($_POST['a'] == 1 && !empty($request_data)) {
            ...
            else if($pt->config->s3_upload == 'on' || $pt->config->ftp_upload == 'on' || $pt->config->spaces == 'on' || $pt->config->wasabi_storage == 'on' || $pt->config->backblaze_storage == 'on'){
...
if ($first == 'test_s3_2') {
    ...
    try {
        ...
        $buckets = $s3->listBuckets();
        if (!empty($buckets) && in_array($pt->config->bucket_name_2, $buckets)) {
            $data['status'] = 200;
        }
...
if ($first == 'ffmpeg_debug') {
    ...
    if (!empty($_FILES['video']['tmp_name'])) {
        ...
        $pt->config->backblaze_storage = 'off';
        $file_upload   = PT_ShareFile($file_info);
        if (!empty($file_upload['filename'])) {
