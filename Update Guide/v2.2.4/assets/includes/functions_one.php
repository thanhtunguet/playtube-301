...
function PT_GetMedia($media = '', $is_upload = false){
    ...
    elseif ($pt->config->backblaze_storage == 'on' && !empty($pt->config->backblaze_bucket_id)) {
        return 'https://f004.backblazeb2.com/file/testnew432/' . $media;
    }
...
function PT_UploadToS3($filename, $config = array()) {
    ...
    elseif ($pt->config->spaces == 'on' && !empty($pt->config->spaces_key) && !empty($pt->config->spaces_secret) && !empty($pt->config->space_name) && !empty($pt->config->space_region)) {
        ...
        $s3 = new S3($pt->config->spaces_key, $pt->config->spaces_secret);

        $s3->setEndpoint($pt->config->space_region.".digitaloceanspaces.com");
        $s3->setRegion($pt->config->space_region);
        $uploaded = $s3->putObjectFile($filename,$pt->config->space_name,$filename,'public-read');
        if ($uploaded == true) {
    ...
    elseif ($pt->config->wasabi_storage == 'on' && !empty($pt->config->wasabi_bucket_name)) {
        ...
        $s3 = new S3($pt->config->wasabi_access_key, $pt->config->wasabi_secret_key);
        $s3->setEndpoint($pt->config->wasabi_bucket_region.'.wasabisys.com');
        $s3->setRegion($pt->config->wasabi_bucket_region);
        $s3::$isWasabi = true;

        $uploaded = $s3->putObjectFile($filename,$pt->config->wasabi_bucket_name,$filename,'public-read');
        if ($uploaded == true) {
            if (empty($config['delete']) && empty($config['amazon'])) {
                @unlink($filename);
            }
    ...
    elseif ($pt->config->backblaze_storage == 'on' && !empty($pt->config->backblaze_bucket_id)) {
        ...
        $s3 = new S3($pt->config->amazone_s3_key, $pt->config->amazone_s3_s_key);
        $s3->setRegion($pt->config->region);

        $uploaded = $s3->putObjectFile($filename,$pt->config->s3_bucket_name,$filename,'public-read');
        if ($uploaded == true) {
            if (empty($config['delete']) && empty($config['amazon'])) {
                $ob = $s3->getObjectInfo($pt->config->s3_bucket_name, $filename);
...
function PT_DeleteFromToS3($filename, $config = array()) {
    ...
    if ($pt->config->amazone_s3_2 == 1) {
        ...
        $s3 = new S3($pt->config->amazone_s3_key_2, $pt->config->amazone_s3_s_key_2);
        $s3->setRegion($pt->config->region_2);

        $delete = $s3->deleteObject($pt->config->bucket_name_2, $filename);
        if ($delete == true) {
            return true;
        }
    }
    ...
    elseif ($pt->config->spaces == 'on' && !empty($pt->config->spaces_key) && !empty($pt->config->spaces_secret) && !empty($pt->config->space_name) && !empty($pt->config->space_region)) {
        ...
        $delete = $s3->deleteObject($pt->config->space_name, $filename);
        if ($delete == true) {
            return true;
        }
        return false;
    ...
    elseif ($pt->config->wasabi_storage == 'on' && !empty($pt->config->wasabi_bucket_name)) {
        ...
        $delete = $s3->deleteObject($pt->config->wasabi_bucket_name, $filename);
        if ($delete == true) {
            return true;
        }
        return false;
    ...
    elseif ($pt->config->backblaze_storage == 'on' && !empty($pt->config->backblaze_bucket_id)) {
        ...
        $delete = $s3->deleteObject($pt->config->s3_bucket_name, $filename);
        if ($delete == true) {
            return true;
        }
    }
